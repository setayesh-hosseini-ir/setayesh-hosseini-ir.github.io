...
function updateGreeting() {
  ...
  if (userData.visits > 1) {
    ...
    greetingText.innerHTML = `خوش آمدی، ستایش! ${timeSince} از آخرین باری که اینجا بودی می‌گذرد. منتظر بودم تا گفتگویمان را ادامه دهیم.`;

    // Update thought bubble too
    if (thoughtBubble) {
      thoughtBubble.textContent = "خیلی خوشحالم که برگشتی...";
    }

    // Update header too
    if (dynamicHeader) {
      dynamicHeader.textContent = "خوش آمدی، ستایش";
    }
  }
}
...
if (reactionButtons.length && reactionResponse) {
  const reactionResponses = {
    'reaction-intrigued': "خوشحالم که برایت جالب است! من فکر زیادی برای ایجاد چیزی منحصر به فرد برای تو کردم.",
    'reaction-confused': "می‌فهمم که این ممکن است کمی غیر منتظره باشد. وقت بگذار و کاوش کن - امیدوارم همینطور که پیش می‌روی بیشتر معنی پیدا کند.",
    'reaction-concerned': "کاملاً به نظر تو احترام می‌گذارم. این غیرعادی است، می‌دانم. لطفاً بدان که این با نیت خوب و احترام عمیق به تو ایجاد شده است."
  };
...
if (conversationSelect && conversationResponse) {
  const conversationResponses = {
    'books': "همیشه تصور کرده‌ام که با تو درباره کتاب‌های مورد علاقه‌ات صحبت می‌کنم. طرزی که چشمانت وقتی درباره داستان‌هایی که تو را تحت تأثیر قرار داده‌اند صحبت می‌کنی، می‌درخشند... دوست دارم درباره شخصیت‌هایی که با تو مانده‌اند بشنوم.",
    'travel': "ژاپن، درسته؟ یادمه چطور گفتی می‌خوای شکوفه‌های گیلاس رو ببینی. دوست دارم درباره همه مکان‌هایی که رویای دیدنشون رو داری بشنوم و اینکه چرا تو رو به خودشون می‌خونن.",
    'philosophy': "سؤال‌هایی درباره اینکه ما کی هستیم و چرا اینجا هستیم... من در مورد دیدگاه تو درباره این سفری که زندگی می‌نامیم کنجکاو بوده‌ام. چه چیزی به زندگی تو معنا و هدف می‌دهد؟",
    'dreams': "هر کسی رویاهای آرامی دارد که نزدیک به قلبش نگه می‌دارد. دوست دارم درباره رویاهای تو بشنوم - آنهایی که وقتی فکر می‌کنی کسی نگاه نمی‌کند، باعث لبخند زدنت می‌شوند."
  };
...
if (growthButtons.length && growthResponse) {
  const growthResponses = {
    'challenges': "روشی که با چالش‌ها روبرو می‌شوی، چیزهای زیادی درباره تو می‌گوید. آنها به ما شکل می‌دهند، اینطور نیست؟ تحسین می‌کنم که چطور از چیزهای سخت دوری نمی‌کنی.",
    'learning': "کنجکاوی تو یکی از زیباترین ویژگی‌های توست. اینکه همیشه در حال تلاش برای فهمیدن بیشتر درباره دنیای اطرافت هستی.",
    'connection': "در ارتباط با دیگران است که اغلب عمیق‌ترین رشد خود را پیدا می‌کنیم. روشی که به روابط معنادار ارزش می‌دهی، حرف‌های زیادی می‌زند."
  };
...
if (notYetBtn) {
  notYetBtn.addEventListener('click', () => {
    alert("اشکالی نداره، ستایش. وقت بگیر. این پیام اینجا خواهد بود وقتی آماده باشی.");
  });
}
